...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      router.push(user ? `/dashboard/${user.role.replace('_', '-')}` : '/');
    } catch {
  ...
  return (
    <Layout>
      <div className="min-h-[80vh] pb-12 flex items-center justify-center bg-background/50 relative overflow-hidden">
          ...
          >
            <Card className="glass border-0 shadow-2xl">
              <CardHeader className="space-y-1 text-center pb-8">
                ...
                <CardDescription>Sign in to your Benkiz Caterers account</CardDescription>
              </CardHeader>
              <CardContent>
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    <FormField control={form.control} name="email" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="name@example.com" className="bg-background/50 backdrop-blur-sm border-border/50 focus:border-primary rounded-xl h-12" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={form.control} name="password" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input type="password" placeholder="••••••••" className="bg-background/50 backdrop-blur-sm border-border/50 focus:border-primary rounded-xl h-12" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />

                    <Button type="submit" className="w-full h-12 rounded-full text-base font-medium" disabled={isSubmitting || authLoading}>
                      {isSubmitting ? (
          ...
          >
            <Card className="glass border-0 bg-secondary/30 backdrop-blur-xl h-full">
              <CardHeader>
                ...
                <CardDescription>
                  Use these test accounts to explore different roles. Click any to prefill.
                </CardDescription>
              </CardHeader>
